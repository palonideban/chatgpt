	...
	if update.Message.IsCommand() {
		...
		default:
			if fromID != config.AdminId {
				bot.Reply(chat.ChatID, update.Message.MessageID, fmt.Sprintf("Cmd yang tidak dikenal /%s", command), false)
				break
			}
...
func gptImage(bot *telegram.Bot, chatID int64, gptClient *gpt.GPTClient, prompt string, config *Config) {
	...
	responsePayload, err := gptClient.CallGPT35([]gpt.Message{
		{Role: "system", Content: "Anda adalah asisten yang menghasilkan deskripsi bahasa alami (prompt) untuk kecerdasan buatan (AI) yang menghasilkan gambar"},
		{Role: "user", Content: fmt.Sprintf("Tolong tingkatkan prompt ini: \"%s\". Jawab dengan prompt yang lebih baik saja.Tetap prompt di sebagian besar 200 karakter.Prompt Anda harus dalam satu kalimat.", prompt)},
	}, "gpt-3.5-turbo", 0.7)
	if err == nil {
...
func gptChat(bot *telegram.Bot, update telegram.Update, gptClient *gpt.GPTClient, config *Config, chat *storage.Chat, fromID int64) {
	...
	response := "Maaf, ada masalah dalam menjawab.Anda bisa mencoba lagi"
	if len(responsePayload.Choices) > 0 {
		response = strings.TrimSpace(responsePayload.Choices[0].Message.Content)
	}
